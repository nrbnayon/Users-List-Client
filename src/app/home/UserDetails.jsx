...
const UserDetails = ({ user }) => {
  if (!user) {
    return <div className="w-full md:w-2/3 p-4">No user selected</div>;
  }
  const svgArr = [
    {
      svg: (
  ...
  return (
    <div className="flex w-full flex-col items-center justify-center space-y-4 rounded-xl bg-white p-8 font-sans shadow-lg ">
      <div className="group relative ring-slate-400 ring-offset-base-100 rounded-full ring ring-offset-2">
        <Image
          src={user?.avatar}
          width={110}
          height={110}
          className="h-[110px] w-[110px] rounded-full bg-slate-500 "
          alt={`${user.profile?.firstName} ${user.profile?.lastName}`}
        />
        <span className="absolute top-3 right-0 h-5 w-5 rounded-full border-[3px] border-white bg-green-500 "></span>
        <span className="absolute top-3 right-0 h-5 w-5 animate-ping rounded-full bg-green-500"></span>
      </div>
